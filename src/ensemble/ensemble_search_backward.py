...
def get_parser():
    ...
    parser.add_argument('--fitness_summary_flag', type=str, default='std', choices=['div', 'mean', 'std', 'sem'])
...
if __name__ == '__main__':
    ...
    filename = f'ensemble_search_backward_{fitness_name}_{cost_name}-step_{util_general.parse_separated_list_comma(gan_steps)}-summary_{fitness_summary_flag}-{eval_backbone}_{post_resizer}_{n_samples}'
    ...
    # Backward Search
    all_gans = set([f"{model}_{step}" for model in gan_models for step in gan_steps])
    df = pd.DataFrame(columns=['step', 'step_comb', 'ensemble', 'fitness', 'n_gans'])

    gan_ensemble_temp = copy.deepcopy(all_gans)
    gan_ensemble_optimal = copy.deepcopy(gan_ensemble_temp)
    obj_gan_ensemble_temp = util_ensemble.backward_objective(
        ensemble=gan_ensemble_temp, fitness_name=fitness_name, summary_flag=fitness_summary_flag, cost_name=cost_name, data_synth=df_synth, data_real=df_real
    )
    ...
    while len(gan_ensemble_optimal) > 2:
        ...
        for i in range(len(gan_ensemble_optimal)):
            ...
            obj_gan_ensemble = util_ensemble.backward_objective(
                ensemble=gan_ensemble, fitness_name=fitness_name, summary_flag=fitness_summary_flag, cost_name=cost_name, data_synth=df_synth, data_real=df_real
            )
