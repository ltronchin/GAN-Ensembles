...
def get_parser():
    ...
    parser.add_argument('--dataset_name', type=str, default='pneumoniamnist',  choices=['retinamnist', 'pneumoniamnist', 'breastmnist'], help='The name of dataset')
    ...
    parser.add_argument('--eval_backbone', type=str, default='resnet_ae_50_breastmnist', help="InceptionV3_torch, SwAV_torch, resnet_ae_50_pneumoniamnist, cnn_resnet_50_pneumoniamnist, disc_resnet_50_pneumoniamnist, resnet_ae_50_retinamnist, cnn_resnet_50_retinamnist, disc_resnet_50_retinamnist, resnet_ae_50_breastmnist, cnn_resnet_50_breastmnist, disc_resnet_50_breastmnist")
    parser.add_argument('--n_samples', type=int, default='4708', help='Total number of images generated by the ensemble.')

    parser.add_argument('--obj_name', type=str, default='intra_inter', choices=['inter__intra', 'inter', 'intra'])
    ...
    parser.add_argument('--n_trial', type=int, default=10000)
...
if __name__ == '__main__':
    ...
    filename = f'ensemble_search_fitness_name__{fitness_name}-obj_name__{obj_name}-step__{util_general.parse_separated_list_comma(gan_steps)}-summary__{fitness_summary_flag}-trial__{n_trial}-{eval_backbone}_{post_resizer}_{n_samples}'
    ...
    func = lambda trial: util_ensemble.objective_pairwise(
        trial=trial,
    ...
    if obj_name == 'intra_inter':
        ...
        # Drop 'params' from each element of the list.
        best_trials_gans = [x.split('params_')[-1] for x in best_trials_gans]
        with open(os.path.join(ensemble_dir, f"best_gans.txt"), 'w') as f:
            f.write(f"\n{util_general.parse_separated_list_comma(best_trials_gans)}")
