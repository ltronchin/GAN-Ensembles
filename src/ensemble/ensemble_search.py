...
if __name__ == '__main__':
    ...
    assert len(gan_steps) == 1 # only one step is allowed
    ...
    filename = f'ensemble_search_{fitness_name}-step_{gan_steps[0]}-summary_{fitness_summary_flag}-cost_name_{cost_name}_trial_{n_trial}-{eval_backbone}_{post_resizer}_{n_samples}'
    ...
    # GANs
    gan_models_step = [f"{x}_{gan_steps[0]}" for x in gan_models]
    search_space = []
    for size in tqdm(range(2, len(gan_models_step) + 1)):
        search_space.extend([util_general.parse_separated_list_comma(x) for x in combinations(gan_models_step, size)])
