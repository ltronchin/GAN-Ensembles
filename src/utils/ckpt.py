...
def load_step_model(ckpt_dir, Gen, Dis, apply_g_ema, Gen_ema, ema, step):
    Gen, Dis, Gen_ema = misc.peel_models(Gen, Dis, Gen_ema)
    Gen_ckpt_path = join(ckpt_dir, f"model=G-current-weights-step={step}.pth")
    Dis_ckpt_path =join(ckpt_dir, f"model=D-current-weights-step={step}.pth")
    print('Load from gen ckpt path: ', Gen_ckpt_path)
    print('Load from dis ckpt path: ', Dis_ckpt_path)
    ...
    if apply_g_ema:
        Gen_ema_ckpt_path =  join(ckpt_dir, f"model=G-current-weights-step={step}.pth")
        load_ckpt(model=Gen_ema,
                  optimizer=None,
                  ckpt_path=Gen_ema_ckpt_path,
                  load_model=True,
