...
def save_model_c(states, mode, RUN):
    ckpt_path = join(RUN.ckpt_dir, "model=C-{mode}-weights.pth".format(mode=mode))     # ckpt_path = join(RUN.ckpt_dir, "model=C-{mode}-best-weights.pth".format(mode=mode))
...
def save_images(data_loader, generator, discriminator, is_generate, num_images, y_sampler, batch_size, z_prior,
    ...
    if is_generate:
        ...
        directory = join(directory, image_type) # added
            ...
            for idx, img in enumerate(images.detach()):
                if batch_size * i + idx < num_images:
                    if save_fake_as_tiff:
                        ...
                        # im = ((img + 1) / 2).clamp(0.0, 1.0)
                        im = img.detach().cpu().numpy()[0]
                        im = Image.fromarray(im)
                        im.save(join(directory, str(labels[idx].item()), "{idx}.tiff".format(idx=batch_size * i + idx)), compression=None)
                        # Reload
